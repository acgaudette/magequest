...
#define DEFAULT_VMOD 10
#define DEFAULT_LVLVAR 0.3f
#define DEFAULT_STATVAR 0.25f

class Roller;

// Base entity class, representing beings or objects, including the player--
// not abstract because instantiating an Entity is a valid action
class Entity {
...
protected:
  ...
  // Function for overloaded '<<' operator, separate from overloaded operator
  // function below to support polymorphism (virtual)
...
public:
  ...
  // The base vitality modifier is multiplied by the entity level to get the
  // final max vitality
  ...
  virtual bool LevelUp(const int delta = 1);
  void ApplyVitality(const int delta);

  // Member function so that PrintToStream can stay protected vs. public,
  // friend so that 'this' doesn't interfere with the operator overload
  friend ostream& operator<<(ostream &os, const Entity &e);

  // Generate a random stat given a base value and
  // factor of variation in either direction
